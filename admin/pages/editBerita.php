...
$q = $dbh->prepare("SELECT Judul, Konten FROM WDberita WHERE ID=? LIMIT 1");
$q->execute([$id]);
...
if (filter_has_var(INPUT_POST, 'judul')) {
    ...
    $u = $dbh->prepare("UPDATE WDberita SET Judul=?, Konten=? WHERE ID=?");
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post">
                ...
                <div class="form-group mb-3">
                    <label>Konten</label>
                    <textarea name="konten" class="form-control" rows="6" required><?= $d['Konten']; ?></textarea>
                </div>

                <button class="btn btn-primary">Simpan Perubahan</button>
                <a href="?x=0.3.4.0.0.0" class="btn btn-secondary">Kembali</a>
